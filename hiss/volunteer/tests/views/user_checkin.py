...
class CheckinUserViewTestCase(TokenAuthTestCase):
    ...
    def test_get_succeeds_for_volunteer(self):
        ...
        expected_output = {
            ...
            "last_name": "last",
            "wares": "None",
            "checkinStatus": STATUS_CHECKED_IN,
        }
        app.first_name = expected_output["first_name"]
        app.last_name = expected_output["last_name"]
        if app.wares:
            app.wares = expected_output["wares"]
        app.status = expected_output["checkinStatus"]
        app.save()
